...
import {
  X, Download, Building, Calendar, FileText, AlertCircle, Loader2, Store, ChevronDown
} from 'lucide-react';
import {
  BANK_OPTIONS,
...
export default function ExportModal({
  ...
  // 自動從申請單的門店帶入銀行帳戶
  useEffect(() => {
    const autoSelectStoreFromRequests = async () => {
      if (!isOpen || requests.length === 0 || stores.length === 0) return;

      // 取得所有申請單的門店名稱（去重）
      const storeNames = [...new Set(requests.map(r => r.store).filter(Boolean))];
      ...
      if (storeNames.length === 1) {
        ...
        if (matchedStore && matchedStore.code !== selectedStoreCode) {
          console.log(`🏪 自動帶入門店: ${matchedStore.code} - ${matchedStore.name}`);
          setSelectedStoreCode(matchedStore.code);
        }
      }
    };

    autoSelectStoreFromRequests();
  }, [isOpen, requests, stores]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // 重置門市選擇 - 會在 autoSelectStoreFromRequests 中根據申請單自動帶入
  ...
  const totalAmount = requests.reduce((sum, req) => {
    return sum + Math.round(Number(req.amount || req.total_amount || 0));
  }, 0);
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-200">
            <div className="text-sm font-bold text-emerald-800 mb-3 flex items-center gap-2">
              <Store size={16} />
              從門市帶入付款方資訊
              {selectedStoreCode && requests.length > 0 && (
