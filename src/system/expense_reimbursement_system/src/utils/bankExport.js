...
function generateTaishinRow(request, paymentDate) {
  const fields = [
    formatDate(paymentDate),                              // 1. 付款日期 YYYYMMDD
    (request.bank_code || '').slice(0, 3),                // 2. 收款銀行碼 (3碼)
    (request.branch_code || '').slice(0, 4),              // 3. 收款分行碼 (4碼)
    request.account_number || '',                          // 4. 收款帳號
    request.applicant_name || request.payee_name || '',    // 5. 收款人戶名 (申請人姓名)
    Math.round(Number(request.total_amount || 0)),         // 6. 交易金額
  ];
  return fields.join('\t');
}
...
function generateCathayRow(request, paymentDate, companyInfo = {}) {
  ...
  const fields = [
    ...
    formatDate(paymentDate),                                // 3. 匯款日期 (8) YYYYMMDD
    'SPU',                                                  // 4. 交易代碼 (3)
    ...
    'TWD',                                                  // 10. 幣別 (3)
    '+',                                                    // 11. 符號 (1)
    formatAmount(request.total_amount || 0, 12, 2),         // 12. 匯款金額 (14)
    ...
    padRight(request.applicant_name || request.payee_name, 70), // 16. 收款人戶名 (70)
    '0',                                                    // 17. 通知方式 (1) 0:不通知
    padRight('', 50),                                       // 18. 通知號碼 (50)
    '15',                                                   // 19. 手續費負擔 (2) 15:外加
    '0000',                                                 // 20. 保留欄位 (4)
    padRight('員工代墊款', 50),                              // 21. 附言/備註 (50)
  ];
