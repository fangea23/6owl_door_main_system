...
--
-- 問題：
-- view 錯誤使用 payment_approval.accountant_brands
-- 但表實際在 public.accountant_brands
-- 導致查詢失敗
--
-- 解決方案：
-- 重建所有會計相關的 view，使用正確的 public.accountant_brands
...
FROM payment_approval.payment_requests pr
  ...
  JOIN public.accountant_brands ab ON (b.id = ab.brand_id)  -- ✅ 使用 public schema
...
-- 說明：
-- accountant_brands 表在 public schema
-- 所有引用都必須使用 public.accountant_brands
