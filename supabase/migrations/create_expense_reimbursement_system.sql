...
CREATE TABLE IF NOT EXISTS public.expense_reimbursement_requests (
  ...
  status TEXT NOT NULL DEFAULT 'draft' CHECK (
    status IN ('draft', 'pending_ceo', 'pending_boss', 'pending_audit_manager', 'approved', 'rejected', 'cancelled')
  ),
...
COMMENT ON COLUMN public.expense_reimbursement_requests.status IS '狀態：draft(草稿), pending_ceo(待總經理簽核), pending_boss(待放行主管簽核), pending_audit_manager(待審核主管簽核), approved(已核准), rejected(已駁回), cancelled(已取消)';
...
COMMENT ON COLUMN public.expense_approvals.approval_type IS '簽核類型：ceo(總經理), boss(放行主管), audit_manager(審核主管)';
